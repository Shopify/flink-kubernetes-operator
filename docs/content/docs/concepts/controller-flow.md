...
**setRandomApplicationResultStorePath**: Flink 2.3 and above will not submit an application for which a terminal result already exists. The Application ID is derived from the clusterId and does not rotate between deployments like the jobid does, so a result retained by a previous deployment blocks the next one, silently since we keep the JobManager alive. We therefore create a random application result store path per deployment, as described here: https://issues.apache.org/jira/browse/FLINK-40467 . Users need to manually clean up the applicationresultstore later.
